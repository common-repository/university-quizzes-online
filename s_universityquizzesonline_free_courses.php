...
$qfree_qms_id_count = intval($_REQUEST['id_course']);
$qfree_qms_id_ready = intval($_REQUEST['ready_course']);
?>
<style>
a.wp_qmsa_course_link1,
a.wp_qmsa_course_link1:link,
a.wp_qmsa_course_link1:visited
{
...
a.wp_qmsa_course_link1:hover,
a.wp_qmsa_course_link1:active
{
...
if(empty($qfree_qms_id_count))
{
    $wp_qmsa_course_free_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM $qfree_qms_table_courses where q_s_active = %d", $wp_qmsa_course_active ));
    
    if($wp_qmsa_course_free_count == 0)
    {
        ?>
        <div style="padding: 30px 20px; font-size: 20px; border: 1px solid #c3c4c7; text-align: center;">
            <b>You don't have any courses created!</b>    
        </div>
        <?php
    }
    else
    {
        ?>
        <div style="width: 100%;">
            <div style="padding: 10px; font-size: 18px; color: #000000; background: #ffd699; border: 1px solid #ffd699;">
                <b>Course Name</b> <span style="font-size: 14px;"> - Click on the course name to start the quiz</span> 
            </div>
        </div>

        <?php
        $wp_qmsa_course_free_sql = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $qfree_qms_table_courses where q_s_active = %d ", $wp_qmsa_course_active));
        foreach($wp_qmsa_course_free_sql as $wp_qmsa_course_free_sqlD)
        {
            $wp_qmsa_course_free_id = $wp_qmsa_course_free_sqlD->q_id_course;
            $wp_qmsa_course_free_name = $wp_qmsa_course_free_sqlD->q_name_course;
            ?>
            <div style="width: 100%; border: 1px solid #d9d9d9;">
                <div style="padding: 10px;">
                    <a href="?id_course=<?php echo $wp_qmsa_course_free_id;?>" class="wp_qmsa_course_link1"><?php echo $wp_qmsa_course_free_name;?></a>
                </div>
...
if(!empty($qfree_qms_id_count) && $qfree_qms_id_ready !=1)
{
                ...
                <div id="wp_qmsa_question_resultsW" style="display: none;">
                    ...
                    <div class="wp_qmsa_question_ResultWindow" style="margin: 15px 0px 0px 0px; min-height: 100px;  color: #000000; border: 1px solid #ffd699; border-radius: 10px; background: #ffd699;">
                        ...
                        <div style="padding: 0px 0px 20px 0px;">    
                            ...
                            <div style="padding: 15px 0px 0px 0px; text-align: center;">
                                <a href="?id_course=x" class="wp_qmsa_course_link1"><b>Close</b></a>
                            </div>
